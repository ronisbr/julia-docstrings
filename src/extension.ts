 ...
 * @returns Joined function declaration or empty string if invalid.
 */
...
export function activate(context: vscode.ExtensionContext) {
    const disposable = vscode.commands.registerCommand(
        ...
        async () => {
            ...
            // If VSCodeVim is active, we need to switch to insert mode at the start of the
            // line. Otherwise the snippet insertion may not work as expected.
            if (vimActive) {
            ...
            try {
                ...
                vscode.window.showErrorMessage(
                    `Failed to insert docstring: ${error instanceof Error ? error.message : String(error)}`
                );
            }
        }
    );
